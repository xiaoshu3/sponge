...
void TCPConnection::segment_received(const TCPSegment &seg) {
    ...
    if(seg.header().seqno == _receiver.ackno() && seg.length_in_sequence_space() && _segments_out.empty()){
        _sender.send_empty_segment();
    }
    if(_receiver._out_of_window(seg) && _segments_out.empty()){
        _sender.send_empty_segment();
    }
    if(seg.header().ack){
        _sender.ack_received(seg.header().ackno,seg.header().win);
...
void TCPConnection::push_segment_out(){
    ...
    while(!_sender.segments_out().empty()){
        ...
        seg.header().win = max(numeric_limits<uint16_t>::max(),static_cast<uint16_t>(_receiver.window_size()));
